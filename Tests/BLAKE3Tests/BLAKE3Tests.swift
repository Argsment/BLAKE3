...
// Test input: repeating sequence 0, 1, 2, ..., 249, 250, 0, 1, ...
func makeTestInput(length: Int) -> [UInt8] {
    var input = [UInt8](repeating: 0, count: length)
    for i in 0..<length {
        input[i] = UInt8(i % 251)
    }
    return input
...
// Test key: "whats the Elvish word for friend" (32 ASCII bytes)
let testKey: [UInt8] = Array("whats the Elvish word for friend".utf8)

// Test context for derive_key
let testContext = "BLAKE3 2019-12-27 16:29:52 test vectors context"
...
// Official test vectors from the BLAKE3 spec
struct TestCase {
    let inputLen: Int
    let hash: String        // hex, 131 bytes (extended output)
    let keyedHash: String   // hex, 131 bytes
    let deriveKey: String   // hex, 131 bytes
}
...
final class BLAKE3Tests: XCTestCase {
    ...
    // Test all hash vectors (default 32-byte output)
    func testHashDefault() {
        for tc in testCases {
            let input = makeTestInput(length: tc.inputLen)
            var hasher = BLAKE3Hasher()
            hasher.update(input)
            let result = hasher.finalize()
            let expected = Array(hexToBytes(tc.hash)[0..<32])
            XCTAssertEqual(result, expected,
                           "hash mismatch for input_len=\(tc.inputLen)")
        }
    }

    // Test extended output (131 bytes)
    func testHashExtendedOutput() {
        for tc in testCases {
            let input = makeTestInput(length: tc.inputLen)
            var hasher = BLAKE3Hasher()
            hasher.update(input)
            let result = hasher.finalize(outputLength: 131)
            let expected = hexToBytes(tc.hash)
            XCTAssertEqual(result, expected,
                           "extended hash mismatch for input_len=\(tc.inputLen)")
        }
    }

    // Test keyed hash (default 32-byte output)
    func testKeyedHashDefault() {
        for tc in testCases {
            let input = makeTestInput(length: tc.inputLen)
            var hasher = BLAKE3Hasher(key: testKey)
            hasher.update(input)
            let result = hasher.finalize()
            let expected = Array(hexToBytes(tc.keyedHash)[0..<32])
            XCTAssertEqual(result, expected,
                           "keyed_hash mismatch for input_len=\(tc.inputLen)")
        }
    }

    // Test keyed hash extended output (131 bytes)
    func testKeyedHashExtendedOutput() {
        for tc in testCases {
            let input = makeTestInput(length: tc.inputLen)
            var hasher = BLAKE3Hasher(key: testKey)
            hasher.update(input)
            let result = hasher.finalize(outputLength: 131)
            let expected = hexToBytes(tc.keyedHash)
            XCTAssertEqual(result, expected,
                           "extended keyed_hash mismatch for input_len=\(tc.inputLen)")
        }
    }

    // Test derive_key (default 32-byte output)
    func testDeriveKeyDefault() {
        for tc in testCases {
            let input = makeTestInput(length: tc.inputLen)
            var hasher = BLAKE3Hasher(derivingKeyFromContext: testContext)
            hasher.update(input)
            let result = hasher.finalize()
            let expected = Array(hexToBytes(tc.deriveKey)[0..<32])
            XCTAssertEqual(result, expected,
                           "derive_key mismatch for input_len=\(tc.inputLen)")
        }
    }

    // Test derive_key extended output (131 bytes)
    func testDeriveKeyExtendedOutput() {
        for tc in testCases {
            let input = makeTestInput(length: tc.inputLen)
            var hasher = BLAKE3Hasher(derivingKeyFromContext: testContext)
            hasher.update(input)
            let result = hasher.finalize(outputLength: 131)
            let expected = hexToBytes(tc.deriveKey)
            XCTAssertEqual(result, expected,
                           "extended derive_key mismatch for input_len=\(tc.inputLen)")
        }
    }

    // Test incremental updates produce the same result as single update
    func testIncrementalUpdate() {
        let input = makeTestInput(length: 6000)

        // Single update
        var hasher1 = BLAKE3Hasher()
        hasher1.update(input)
        let result1 = hasher1.finalize()

        // Many small updates
        var hasher2 = BLAKE3Hasher()
        var offset = 0
        let chunkSizes = [1, 2, 3, 5, 7, 11, 13, 17, 64, 65, 100, 1024, 1025]
        var sizeIdx = 0
        while offset < input.count {
            let take = min(chunkSizes[sizeIdx % chunkSizes.count], input.count - offset)
            hasher2.update(Array(input[offset..<offset + take]))
            offset += take
            sizeIdx += 1
        }
        let result2 = hasher2.finalize()

        XCTAssertEqual(result1, result2, "Incremental update mismatch")
    }

    // Test reset
    func testReset() {
        let input = makeTestInput(length: 256)
        var hasher = BLAKE3Hasher()
        hasher.update(input)
        ...
        XCTAssertEqual(result1, result2, "Reset should produce same result")
    }

    // Test convenience function
    func testConvenienceFunction() {
        let input = makeTestInput(length: 0)
        let result = blake3Hash(input)
        let expected = Array(hexToBytes(testCases[0].hash)[0..<32])
        XCTAssertEqual(result, expected, "Convenience function mismatch")
    }
    ...
    func testEmptyInput() {
        ...
        let result = hasher.finalize()
        let expected = Array(hexToBytes(testCases[0].hash)[0..<32])
        XCTAssertEqual(result, expected, "Empty input mismatch")
    }

    // Test finalize_seek
    func testFinalizeSeek() {
        let input = makeTestInput(length: 256)
        var hasher = BLAKE3Hasher()
        hasher.update(input)

        let fullOutput = hasher.finalize(outputLength: 200)
        let seekOutput = hasher.finalize(seek: 100, outputLength: 100)

        XCTAssertEqual(Array(fullOutput[100..<200]), seekOutput,
                       "Seek output should match offset in full output")
    }
}
